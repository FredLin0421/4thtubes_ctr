...
class DesiredpointsComp(ExplicitComponent):
    ...
    def compute_partials(self,inputs,partials):
        ...
        pd_pt[:,4] = (rot_p[interpolation_idx_r,np.arange(k),1] - rot_p[interpolation_idx_l,np.arange(k),1]).squeeze()
        pd_pt[:,8] = (rot_p[interpolation_idx_r,np.arange(k),2] - rot_p[interpolation_idx_l,np.arange(k),2]).squeeze()
...
if __name__ == '__main__':
    ...
    comp.add_output('tube_ends_tip', val=([1.3,3,3,4],[2.1,3.1,3.1,4]))
    ...
    comp = DesiredpointsComp(num_nodes=n,k=k,tube_nbr=4)
