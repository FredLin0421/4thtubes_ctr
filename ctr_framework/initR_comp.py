...
class InitialRComp(ExplicitComponent):
    ...
    def compute_partials(self,inputs,partials):
        ...
        Pr_ppsi = np.zeros((k*3*3, num_nodes*k*tube_nbr))
        k_idx = np.arange(k)
        n_idx = np.arange(k)
        Pr_ppsi[k_idx*9,n_idx*tube_nbr] = -np.sin(psi[0,:,0])
        Pr_ppsi[k_idx*9+1,n_idx*tube_nbr] = -np.cos(psi[0,:,0])
        Pr_ppsi[k_idx*9+3,n_idx*tube_nbr] = np.cos(psi[0,:,0])
        Pr_ppsi[k_idx*9+4,n_idx*tube_nbr] = -np.sin(psi[0,:,0])
...
if __name__ == '__main__':
    ...
    comp.add_output('psi',val = np.random.random((num_nodes,k,tube_nbr)))
  
    group.add_subsystem('comp1', comp, promotes = ['*'])
    
    
    comp = InitialRComp(num_nodes=num_nodes,k=k,tube_nbr=tube_nbr)
